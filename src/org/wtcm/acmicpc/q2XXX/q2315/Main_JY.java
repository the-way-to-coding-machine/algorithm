...
//                int outer = W_ACC[N] - W_ACC[end] + W_ACC[start];
//                if (start < N)
...
//        System.out.println(Math.min(dp[1][N][LEFT], dp[1][N][RIGHT]));
    ...
    public static int turnOff(int start, int end, int position) {
        if (start == 1 && end == N) {
            return 0;
        }
        ...
        if (start - 1 >= 1) {
            left = turnOff(start - 1, end, LEFT) + (D[current] - D[start - 1]) * outer;
        }
        if (end + 1 <= N) {
            right = turnOff(start, end + 1, RIGHT) + (D[end + 1] - D[current]) * outer;
        }
